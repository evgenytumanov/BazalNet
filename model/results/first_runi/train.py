...
def main():
    ...
    # construct layers
    output_probs = construct_layers(input_seq) 
    ouput_seq_rsh = tf.reshape(output_probs, [-1, config.numbers_alphabet_size + 1])
    ...
    try:
        with tf.train.MonitoredTrainingSession(checkpoint_dir=experiment_dir,
                                               ...
                                               save_summaries_steps=10, config=tf_config) as sess:

            global_step_init = 0 #get_global_step_init(experiment_dir) #TODO
            ...
            for inputs, targets in utils.iterate_minibatches_global(X_train, y_train, batchsize=config.batch_size, start_it=0, iters=config.iters):

                train_loss = sess.run([train_op, loss, accuracy], feed_dict={input_seq : inputs, ouput_seq : targets})
                ...
                if gs % 20 == 0:
                    val_loss, val_accuracy = calc_validation_loss(sess, loss, accuracy, inputs, ouput_seq, X_val, y_val)
